...
while run:
    ...
    while menu:
        ...
        button = pygame.transform.scale(button, (300, 125))
        cntn = f_end.render('CONTINUE', True, (0, 0, 0))
        r2 = cntn.get_rect(center=(w / 2, h / 2))
        r3 = button.get_rect(center=(w / 2, h / 2))
        screen.blit(button, r3)
        screen.blit(cntn, r2)

        # кнопка для замены автомобиля
        color = f_end.render('CHANGE CAR', True, (0, 0, 0))
        r6 = color.get_rect(center=(w / 2, h / 2 + 100))
        r7 = button.get_rect(center=(w / 2, h / 2 + 100))
        screen.blit(button, r7)
        screen.blit(color, r6)

        # кнопка для того, чтобы начать игру заново
        restart = f_end.render('RESTART', True, (0, 0, 0))
        r4 = restart.get_rect(center=(w / 2, h / 2 + 200))
        r5 = button.get_rect(center=(w / 2, h / 2 + 200))
